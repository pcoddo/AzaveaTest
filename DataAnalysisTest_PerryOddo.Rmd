...
# Produce plot
png("Figures/Figure1_pairsPlot.png", width = 6.5, height = 5.5, units = 'in', res = 300)
f1 = ggpairs(dfPlot,
...
# Create test set from continuous predictor variables
df_test = df[1:40, 3:6]

# Create training set from continuous predictor variables
df_train = df[41:nrow(df), 3:6]
```
...
df_test_target = df[1:40, 10]
df_train_target = df[41:nrow(df), 10]

# Set k value
k = floor(sqrt(nrow(df)))

# Create model
kmod = knn(train = df_train, test = df_test, cl = df_train_target, k = k)

# Create table to visualize model results
test <- table(df_test_target, kmod)
```
```{r, echo=FALSE, results='asis'}
library(knitr)
kable(test, caption = "K-nearest neighbor output", row.names = "Original ")
```